            ...
            <tbody>
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'actions'" style="text-align:center;">
                            <div v-if="item.status === '1'"><i-button :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</i-button></div>
                            <div v-if="item.status === '0'"><i-button :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actionn)' :key="action.text">{{action.text}}</i-button></div>
                        </div>
                        ...
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                ...
                                <i v-if="item.children&&item.children.length>0" class="ivu-icon" :class="{'ivu-icon-plus':!item.expanded,'ivu-icon-minus':item.expanded }"></i>
    ...
    methods: {
      ...
      iconRow() {
        ...
        RowClick(data, event, index, text) {
            ...
            console.log(this.initItems[index].status);
            let that = this;
            if(text == '已停用'){
               that.initItems[index].status = '1';

               that.modify(data.id,"1");
            }else if(text == '已启用'){
               that.initItems[index].status = '0';  

               that.modify(data.id,"0");
            }
            
            // this.$set(this.initItems[index])
        },
        // 点击事件 返回数据处理
        makeData(data) {
